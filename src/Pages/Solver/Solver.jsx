...
const Solver = () => {
    ...
    const solveExpression = async () => {
        ...
        try {
            ...
            // Fetch steps
            const stepsResponse = await fetch("http://127.0.0.1:8000/steps", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ expression: input }),
            });
                ...
                <div className="flex flex-col w-2/4 mx-auto">
                    <div className="flex flex-row space-x-4 mt-10">
                        ...
                        <div className="bg-bt2 rounded-2xl p-4 space-x-10 w-1/2">
                            <InputContainer handleInputChange={handleInputChange} formatForMathJax={formatForMathJax} input={input} solveExpression={solveExpression}/>
                        </div>
                ...
                <div className="flex justify-center items-center mt-4 w-2/4 mx-auto">
                    <div className="bg-bt3 rounded-2xl p-4 space-x-10 w-full flex">
                        <div className="flex flex-col space-y-4 w-full">
                            ...
                            <MathJax>{`$$${derivative}$$`}</MathJax>
                            </div>
                            <strong className="text-tx1 font-bold text-md">Step-by-Step Process</strong>
                            <div className="bg-ct2 rounded-2xl p-4 flex items-center justify-center">
                                <h>Step-by-Step Solution with hover effect describing the rules used in each step</h>
                                <MathJax>{`$$${steps}$$`}</MathJax>
                            </div>
                        </div>
                    </div>
                </div>
